...
void ScriptCore::removeAllRoots(JSContext *cx) {
...
void ScriptCore::createGlobalContext() {
    if (this->m_context && this->rt_) {
        ScriptCore::removeAllRoots(this->m_context);
    ...
    this->m_context = JS_NewContext(rt_, 8192);
    JS_SetOptions(this->m_context, JSOPTION_TYPE_INFERENCE);
...
JSBool ScriptCore::runScript(const char *path, JSObject* global, JSContext* cx)
{
    if (!path) {
        return false;
    }
...
void ScriptCore::removeScriptObjectByCCObject(CCObject* pObj)
{
    js_proxy_t* nproxy;
    js_proxy_t* jsproxy;
    void *ptr = (void*)pObj;
    ...
    if (nproxy) {
        JSContext *cx = ScriptCore::getInstance()->getGlobalContext();
...
JSBool ScriptCore::executeScript(JSContext *cx, uint32_t argc, jsval *vp)
{
    if (argc >= 1) {
        ...
        if (argc == 2 && argv[1].isString()) {
            ...
            if (rootedGlobal) {
                res = ScriptCore::getInstance()->runScript(path, rootedGlobal->get());
            } else {
            ...
            res = ScriptCore::getInstance()->runScript(path, glob);
        }
        return res;
    }
    return JS_TRUE;
...
void ScriptCore::pauseSchedulesAndActions(js_proxy_t* p)
{
...
void ScriptCore::resumeSchedulesAndActions(js_proxy_t* p)
{
    CCArray * arr = JSScheduleWrapper::getTargetForJSObject(p->obj);
    if(!arr) return;
...
void ScriptCore::cleanupSchedulesAndActions(js_proxy_t* p)
{
...
int ScriptCore::executeNodeEvent(CCNode* pNode, int nAction)
{
    js_proxy_t * p;
...
int ScriptCore::executeMenuItemEvent(CCMenuItem* pMenuItem)
{
    js_proxy_t * p;
    ...
    executeJSFunctionFromReservedSpot(this->m_context, p->obj, dataVal, retval);

    return 1;
}

int ScriptCore::executeNotificationEvent(CCNotificationCenter* pNotificationCenter, const char* pszName)
{
    return 1;
}

int ScriptCore::executeCallFuncActionEvent(CCCallFunc* pAction, CCObject* pTarget/* = NULL*/)
{
    return 1;
}

int ScriptCore::executeSchedule(int nHandler, float dt, CCNode* pNode/* = NULL*/)
{
    js_proxy_t * p;
    JS_GET_PROXY(p, pNode);
...
int ScriptCore::executeLayerTouchesEvent(CCLayer* pLayer, int eventType, CCSet *pTouches)
{
    std::string funcName = "";
    getTouchesFuncName(eventType, funcName);

    JSObject *jsretArr = JS_NewArrayObject(this->m_context, 0, NULL);

    JS_AddNamedObjectRoot(this->m_context, &jsretArr, "touchArray");
    int count = 0;
    for(CCSetIterator it = pTouches->begin(); it != pTouches->end(); ++it, ++count) {
        jsval jsret;
        getJSTouchObject(this->m_context, (CCTouch *) *it, jsret);
        if(!JS_SetElement(this->m_context, jsretArr, count, &jsret)) {
            break;
        }
    }
    ...
    JS_RemoveObjectRoot(this->m_context, &jsretArr);

    for(CCSetIterator it = pTouches->begin(); it != pTouches->end(); ++it, ++count) {
        jsval jsret;
        removeJSTouchObject(this->m_context, (CCTouch *) *it, jsret);
    }

    return 1;
}

int ScriptCore::executeLayerTouchEvent(CCLayer* pLayer, int eventType, CCTouch *pTouch)
{
    std::string funcName = "";
...
bool ScriptCore::executeFunctionWithObjectData(CCNode *self, const char *name, JSObject *obj) {
...
JSBool ScriptCore::executeFunctionWithOwner(jsval owner, const char *name, uint32_t argc /* = 0 */, jsval *vp /* = NULL */, jsval* retVal /* = NULL */)
{
    JSBool bRet = JS_FALSE;
    JSBool hasAction;
    jsval temp_retval;
    JSContext* cx = this->m_context;
...
int ScriptCore::executeAccelerometerEvent(CCLayer *pLayer, CCAcceleration *pAccelerationValue) {
...
int ScriptCore::executeLayerKeypadEvent(CCLayer* pLayer, int eventType)
{
	js_proxy_t * p;
...
int ScriptCore::executeCustomTouchesEvent(int eventType,
...
int ScriptCore::executeCustomTouchEvent(int eventType,
    ...
    jsval jsTouch;
    getJSTouchObject(this->m_context, pTouch, jsTouch);
    ...
    removeJSTouchObject(this->m_context, pTouch, jsTouch);
    return 1;
...
void SimpleRunLoop::update(float dt) {
    ...
    while (queue.size() > 0) {
        ...
        string str = *first;
        ScriptCore::getInstance()->debugProcessInput(str);
        queue.erase(first);
    }
    pthread_mutex_unlock(&g_qMutex);
}

void ScriptCore::debugProcessInput(string str) {
    JSString* jsstr = JS_NewStringCopyZ(m_context, str.c_str());
    jsval argv[3] = {
    ...
    jsval outval;
    JSAutoCompartment ac(m_context, m_debugGlobal);
    JS_CallFunctionName(m_context, m_debugGlobal, "processInput", 3, argv, &outval);
}

void ScriptCore::enableDebugger() {
    if (m_debugGlobal == NULL) {
        m_debugGlobal = NewGlobalObject(m_context, true);
        JS_WrapObject(m_context, &m_debugGlobal);
        JSAutoCompartment ac(m_context, m_debugGlobal);
        // these are used in the debug program
        JS_DefineFunction(m_context, m_debugGlobal, "log", ScriptCore::log, 0, JSPROP_READONLY | JSPROP_PERMANENT);
        JS_DefineFunction(m_context, m_debugGlobal, "_bufferWrite", JSBDebug_BufferWrite, 1, JSPROP_READONLY | JSPROP_PERMANENT);
        JS_DefineFunction(m_context, m_debugGlobal, "_bufferRead", JSBDebug_BufferRead, 0, JSPROP_READONLY | JSPROP_PERMANENT);
        JS_DefineFunction(m_context, m_debugGlobal, "_lockVM", JSBDebug_LockExecution, 2, JSPROP_READONLY | JSPROP_PERMANENT);
        JS_DefineFunction(m_context, m_debugGlobal, "_unlockVM", JSBDebug_UnlockExecution, 0, JSPROP_READONLY | JSPROP_PERMANENT);

        runScript("jsb_debugger.js", m_debugGlobal);

        // prepare the debugger
        jsval argv = OBJECT_TO_JSVAL(m_global);
        jsval outval;
        JSBool ok = JS_CallFunctionName(m_context, m_debugGlobal, "_prepareDebugger", 1, &argv, &outval);
        if (!ok) {
            JS_ReportPendingException(m_context);
        }
        // define the start debugger function
        JS_DefineFunction(m_context, m_global, "startDebugger", JSBDebug_StartDebugger, 3, JSPROP_READONLY | JSPROP_PERMANENT);
        // start bg thread
...
{
    JSObject* debugGlobal = ScriptCore::getInstance()->getDebugGlobal();
    if (argc >= 2) {
        jsval* argv = JS_ARGV(cx, vp);
        jsval out;
